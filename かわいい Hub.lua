-- ── GUARD: chỉ load 1 lần mỗi server ───────────────────────────────────────
-- Đợi JobId hợp lệ (Roblox đôi khi chưa gán khi script vừa chạy)
local _waitJobId = game.JobId
if _waitJobId == "" then
    local _t0 = os.clock()
    repeat task.wait(0.1) _waitJobId = game.JobId until _waitJobId ~= "" or os.clock() - _t0 > 5
end
if _G.KawaiiHubJobId == _waitJobId and _waitJobId ~= "" then return end
_G.KawaiiHubJobId = _waitJobId
...
local SCRIPT_URL = "https://raw.githubusercontent.com/taioccho/sexmocbuom/refs/heads/main/%E3%81%8B%E3%82%8F%E3%81%84%E3%81%84%20Hub.lua" -- thay bằng URL raw script của bạn
...
-- Anti-AFK: không dùng VirtualUser, dùng humanoid state thay thế
...
local function clickTauntButton()
    ...
    for _, obj in ipairs(playerGui:GetDescendants()) do
        if (obj:IsA("TextButton") or obj:IsA("ImageButton")) and obj.Visible then
            ...
            if isTaunt then
                pcall(function()
                    -- Kích hoạt qua event connections (không dùng VIM tránh click nhầm UI game)
                    if getconnections then
                        ...
                        -- Fallback nếu không có getconnections: fire trực tiếp signal
...
-- Tìm frame chính của Fluent để reset position
local _hubMainFrame = nil
local function findFluentGui()
    local guiParent
    local ok, h = pcall(function() return gethui() end)
    guiParent = (ok and h) or CoreGui
    for _, sg in ipairs(guiParent:GetChildren()) do
        if sg:IsA("ScreenGui") then
            for _, child in ipairs(sg:GetDescendants()) do
                if child:IsA("Frame") and child.Parent == sg then
                    _hubMainFrame = child
                    return
                end
            end
...
ToggleAutoExecute:OnChanged(function(v)
    _G.AutoExecute = v
    -- Re-queue từ source đã cache khi bật toggle
    pcall(function()
...
SettingsTab:AddButton({
    ...
    Description = "Move the window back to center of the screen",
    Callback    = function()
        pcall(function()
            -- Thử tìm lại nếu chưa tìm được
            if not _hubMainFrame or not _hubMainFrame.Parent then
                findFluentGui()
            end

            if _hubMainFrame and _hubMainFrame.Parent then
                local vp = workspace.CurrentCamera.ViewportSize
                local w = _hubMainFrame.AbsoluteSize.X
                local h2 = _hubMainFrame.AbsoluteSize.Y
                _hubMainFrame.Position = UDim2.fromOffset(
                    math.floor(vp.X / 2 - w / 2),
                    math.floor(vp.Y / 2 - h2 / 2)
                )
            else
                -- Fallback: dò toàn bộ các Frame lớn trong tất cả ScreenGui
                local guiParent
                local ok2, h3 = pcall(function() return gethui() end)
                guiParent = (ok2 and h3) or CoreGui
                local vp = workspace.CurrentCamera.ViewportSize
                for _, sg in ipairs(guiParent:GetChildren()) do
                    if sg:IsA("ScreenGui") then
                        for _, fr in ipairs(sg:GetDescendants()) do
                            if fr:IsA("Frame") and fr.AbsoluteSize.X > 200 and fr.AbsoluteSize.Y > 150 and fr.Parent:IsA("ScreenGui") then
                                fr.Position = UDim2.fromOffset(
                                    math.floor(vp.X / 2 - fr.AbsoluteSize.X / 2),
                                    math.floor(vp.Y / 2 - fr.AbsoluteSize.Y / 2)
                                )
                                break
                            end
                        end
            ...
            if _G.AutoSeeker then
                ...
                if targetHrp and dist <= _G.AttackRange then
                    if os.clock() - lastAttack >= ATTACK_COOLDOWN then
                        -- Tấn công bằng KnifeHit remote trực tiếp
                        pcall(function()
                            game:GetService("ReplicatedStorage"):WaitForChild("Knife"):WaitForChild("Remotes"):WaitForChild("KnifeHit"):FireServer()
                        end)
                ...
                if dist <= 5 then
                    if os.clock() - lastTauntTime >= 5 then
                        ...
                        if not tauntDone then
                            -- Fire connection của phím Q trực tiếp, không dùng VirtualInputManager
                            pcall(function()
                                if getconnections then
                                    local uis = game:GetService("UserInputService")
                                    for _, conn in ipairs(getconnections(uis.InputBegan)) do
                                        conn:Fire(Instance.new("InputObject") and {
