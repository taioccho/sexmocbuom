...
if not getgenv().Config.Team or getgenv().Config.Team == "" then
    if player.Team then
        getgenv().Config.Team = player.Team.Name
    else
...
local EquipWeapon = function(text)
    if not text then return end
    if player.Backpack:FindFirstChild(text) then
        if player.Character and player.Character:FindFirstChildOfClass("Humanoid") then
            player.Character:FindFirstChildOfClass("Humanoid"):EquipTool(player.Backpack:FindFirstChild(text))
        end
    end
end

local weaponSc = function(weapon)
    if player.Character then
        for _, heldTool in pairs(player.Character:GetChildren()) do
