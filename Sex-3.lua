...
Tabs.Stack:AddToggle('Toggle', {
    ...
    -- Dọn dẹp tween Pirate Raid khi tắt
    if not p562 then
        if getgenv()._PirateRaidTween then
            getgenv()._PirateRaidTween:Cancel()
            getgenv()._PirateRaidTween = nil
        end
        getgenv()._PirateRaidTweening = false
        if getgenv()._PirateRaidBodyVel then
            pcall(function() getgenv()._PirateRaidBodyVel:Destroy() end)
            getgenv()._PirateRaidBodyVel = nil
        end
        if getgenv()._PirateRaidBodyPos then
            pcall(function() getgenv()._PirateRaidBodyPos:Destroy() end)
            getgenv()._PirateRaidBodyPos = nil
        end
    end
end)

-- Helper: đếm số trái đang lưu trong storage
local function GetFruitStoredCount()
    ...
    for _, item in pairs(inv) do
        if item.Type == 'Fruit' or (item.Name and string.find(item.Name, 'Fruit')) then
            count = count + 1
        end
    end
    return count
...
-- Helper: lấy max slot trái (mặc định 3, có thể nâng lên 9)
local function GetMaxFruitSlot()
    local ok, result = pcall(function()
        return game:GetService('ReplicatedStorage').Remotes.CommF_:InvokeServer('FruitStorage')
    end)
    if ok and result and result.MaxSlot then
        return result.MaxSlot
    end
    return 3
end

-- Helper: kiểm tra còn slot trống để lưu trái không
local function HasFruitSlotAvailable()
    local stored = GetFruitStoredCount()
    local maxSlot = GetMaxFruitSlot()
    return stored < maxSlot, stored, maxSlot
end

-- Vòng lặp Pirate Raid mới
task.spawn(function()
    local Players = game:GetService('Players')
    ...
    Players.LocalPlayer.CharacterAdded:Connect(function(newChar)
        raidCharacter = newChar
        raidHRP = newChar:WaitForChild('HumanoidRootPart')
        -- Dọn tween cũ khi respawn
        if getgenv()._PirateRaidTween then
            getgenv()._PirateRaidTween:Cancel()
            getgenv()._PirateRaidTween = nil
        end
    ...
    raidCharacter = Players.LocalPlayer.Character or Players.LocalPlayer.CharacterAdded:Wait()
    raidHRP       = raidCharacter:WaitForChild('HumanoidRootPart')

    -- Ghim nhân vật khi đang tween tới quái
    RunService.Stepped:Connect(function()
        if not getgenv().AutoPirateRaid then
            if getgenv()._PirateRaidBodyVel then
                pcall(function() getgenv()._PirateRaidBodyVel:Destroy() end)
                getgenv()._PirateRaidBodyVel = nil
            end
        ...
        if isTweening and raidHRP then
            for _, part in pairs(raidCharacter:GetDescendants()) do
                if part:IsA('BasePart') then part.CanCollide = false end
            end
            if not getgenv()._PirateRaidBodyVel or getgenv()._PirateRaidBodyVel.Parent ~= raidHRP then
                if getgenv()._PirateRaidBodyVel then getgenv()._PirateRaidBodyVel:Destroy() end
                local bv = Instance.new('BodyVelocity')
                bv.Velocity  = Vector3.new(0, 0, 0)
                bv.MaxForce  = Vector3.new(9e9, 9e9, 9e9)
                bv.Parent    = raidHRP
                getgenv()._PirateRaidBodyVel = bv
            end
            if not getgenv()._PirateRaidBodyPos or getgenv()._PirateRaidBodyPos.Parent ~= raidHRP then
                if getgenv()._PirateRaidBodyPos then getgenv()._PirateRaidBodyPos:Destroy() end
                local bp = Instance.new('BodyPosition')
                bp.MaxForce = Vector3.new(0, 9e9, 0)
                bp.Position = raidHRP.Position
                bp.Parent   = raidHRP
                getgenv()._PirateRaidBodyPos = bp
            end
        else
            if getgenv()._PirateRaidBodyVel then
                pcall(function() getgenv()._PirateRaidBodyVel:Destroy() end)
    ...
    -- Bring mobs vào gần mình
    task.spawn(function()
        while task.wait(2) do
            if raidHRP and getgenv().AutoPirateRaid and workspace:FindFirstChild('Enemies') then
                for _, v in pairs(workspace.Enemies:GetChildren()) do
                    if v:IsA('Model') and v:FindFirstChild('Humanoid') and v:FindFirstChild('HumanoidRootPart') then
                        if v.Humanoid.Health > 0 then
                            local dist = (v.HumanoidRootPart.Position - raidHRP.Position).Magnitude
                            if dist <= BRING_RADIUS then
                                v.HumanoidRootPart.CFrame = raidHRP.CFrame * CFrame.new(0, -4, 0)
                            end
                        end
                    end
                end
    ...
    -- Vòng lặp chính: tìm quái → StopTween cũ → Tween mới
    RunService.Heartbeat:Connect(function(dt)
        if not getgenv().AutoPirateRaid or not World3 then return end
        if not raidHRP then return end
        ...
        if workspace:FindFirstChild('Enemies') then
            for _, v in pairs(workspace.Enemies:GetChildren()) do
                if v:IsA('Model') and v:FindFirstChild('Humanoid') and v:FindFirstChild('HumanoidRootPart') then
                    if v.Humanoid.Health > 0 then
                        if (v.HumanoidRootPart.Position - CHECK_CENTER).Magnitude <= CHECK_RADIUS then
                            targetMob = v
                            break
                        end
                    end
        ...
        if targetMob then
            ...
            -- Nếu xa vùng raid → teleport vào
            if (raidHRP.Position - CHECK_CENTER).Magnitude > 1000 then
                -- StopTween trước khi teleport
                if getgenv()._PirateRaidTween then
                    getgenv()._PirateRaidTween:Cancel()
                    getgenv()._PirateRaidTween = nil
                end
                ...
                pcall(function()
                    ...
                    local mag  = (raidHRP.Position - v563.Position).Magnitude
                    if mag > 1500 then
                        if BypassTP then BTP(v563) end
                    else
                        topos(v563)
                    end
                    task.wait(0.1)
                    topos(TELEPORT_STAGE)
                    task.wait(0.1)
                end)
                return
            end
            ...
            -- Tween tới quái
            local targetPos = targetMob.HumanoidRootPart.Position + Vector3.new(0, 10, 0)
            if not lastTargetPos or (targetPos - lastTargetPos).Magnitude > 3 then
                lastTargetPos = targetPos
                local dist = (raidHRP.Position - targetPos).Magnitude
                if dist > 4 then
                    -- StopTween cũ trước khi tạo tween mới
                    if getgenv()._PirateRaidTween then
                        getgenv()._PirateRaidTween:Cancel()
                        getgenv()._PirateRaidTween = nil
                    end
                    isTweening = true
                    getgenv()._PirateRaidTweening = true
                    ...
                    getgenv()._PirateRaidTween = newTween
                    newTween:Play()
            ...
            if isTweening and getgenv()._PirateRaidBodyPos then
                getgenv()._PirateRaidBodyPos.Position = targetPos
            end

            -- Equip vũ khí & haki khi đang farm
            pcall(function()
                AutoHaki()
                EquipWeapon(getgenv().SelectWeapon)
...
spawn(function()
    local _lastStoreCooldown = 0
    while task.wait(0.2) do
        if getgenv().AutoStoreFruit then
            pcall(function()
                if getgenv().AutoStoreFruit then
                    -- Chống spam: cooldown 3 giây giữa mỗi lần store
                    if tick() - _lastStoreCooldown < 3 then return end

                    -- Kiểm tra slot còn trống không
                    local hasSlot, stored, maxSlot = HasFruitSlotAvailable()
                    if not hasSlot then
                        -- Full slot, không store
                        return
                    end
                    ...
                    while true do
                        ...
                        if _Character15:FindFirstChild(v747) or _Backpack9:FindFirstChild(v747) then
                            -- Kiểm tra lại slot trước mỗi trái (tránh store quá slot)
                            local stillHasSlot = HasFruitSlotAvailable()
                            if not stillHasSlot then break end
                            game:GetService('ReplicatedStorage').Remotes.CommF_:InvokeServer('StoreFruit', v748, _Backpack9:FindFirstChild(v747))
                            task.wait(0.3) -- delay nhỏ giữa các lần store
                        end
                    end
                    _lastStoreCooldown = tick() -- reset cooldown sau khi store xong
                end
            end)
        end
...
-- ============================================================
-- TOGGLE UI | Left Control | Flip Animation | Toggle Window
-- ============================================================
do
    ...
    -- Tìm Window Fluent để toggle ẩn/hiện
    -- Fluent lưu GUI chính trong PlayerGui với tên "Fluent" hoặc frame con của nó
    local function _getFluentGui()
        for _, gui in pairs(_PGui:GetChildren()) do
            if gui:IsA("ScreenGui") and gui.Name ~= "ToggleButtonGui" and gui.Name ~= "Main" then
                return gui
            end
        end
        return nil
    ...
    -- Tạo ScreenGui nút toggle
    local _ScreenGui = Instance.new("ScreenGui")
    _ScreenGui.Name = "ToggleButtonGui"
    _ScreenGui.ResetOnSpawn = false
    _ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    _ScreenGui.IgnoreGuiInset = true
    _ScreenGui.Parent = _PGui

    -- Frame chứa nút (draggable)
    local _BtnFrame = Instance.new("Frame")
    _BtnFrame.Name = "ButtonFrame"
    _BtnFrame.Size = UDim2.new(0, 80, 0, 80)
    _BtnFrame.Position = UDim2.new(0, 20, 0.5, -40)
    _BtnFrame.BackgroundTransparency = 1
    _BtnFrame.AnchorPoint = Vector2.new(0, 0)
    _BtnFrame.Parent = _ScreenGui

    -- Mặt trước (ON - Window hiện)
    local _FrontFace = Instance.new("ImageLabel")
    _FrontFace.Size = UDim2.new(1, 0, 1, 0)
    _FrontFace.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    _FrontFace.BorderSizePixel = 0
    _FrontFace.Image = "rbxassetid://5009915795"
    _FrontFace.ImageColor3 = Color3.fromRGB(255, 255, 255)
    _FrontFace.ScaleType = Enum.ScaleType.Fit
    _FrontFace.Visible = true
    _FrontFace.Parent = _BtnFrame

    local _FrontCorner = Instance.new("UICorner")
    _FrontCorner.CornerRadius = UDim.new(1, 0)
    _FrontCorner.Parent = _FrontFace

    local _FrontStroke = Instance.new("UIStroke")
    _FrontStroke.Color = Color3.fromRGB(0, 200, 100)
    _FrontStroke.Thickness = 2.5
    _FrontStroke.Parent = _FrontFace

    -- Mặt sau (OFF - Window ẩn)
    local _BackFace = Instance.new("ImageLabel")
    _BackFace.Size = UDim2.new(1, 0, 1, 0)
    _BackFace.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    _BackFace.BorderSizePixel = 0
    _BackFace.Image = "rbxassetid://7072706620"
    _BackFace.ImageColor3 = Color3.fromRGB(180, 180, 180)
    _BackFace.ScaleType = Enum.ScaleType.Fit
    _BackFace.Visible = false
    _BackFace.Parent = _BtnFrame

    local _BackCorner = Instance.new("UICorner")
    _BackCorner.CornerRadius = UDim.new(1, 0)
    _BackCorner.Parent = _BackFace

    local _BackStroke = Instance.new("UIStroke")
    _BackStroke.Color = Color3.fromRGB(180, 50, 50)
    _BackStroke.Thickness = 2.5
    _BackStroke.Parent = _BackFace

    -- Label trạng thái
    local _StatusLbl = Instance.new("TextLabel")
    _StatusLbl.Size = UDim2.new(0, 80, 0, 20)
    _StatusLbl.Position = UDim2.new(0, 0, 1, 4)
    _StatusLbl.BackgroundTransparency = 1
    _StatusLbl.Text = "ON"
    _StatusLbl.TextColor3 = Color3.fromRGB(0, 255, 120)
    _StatusLbl.TextScaled = true
    _StatusLbl.Font = Enum.Font.GothamBold
    _StatusLbl.Parent = _BtnFrame

    -- Label phím
    local _KeyLbl = Instance.new("TextLabel")
    _KeyLbl.Size = UDim2.new(0, 80, 0, 16)
    _KeyLbl.Position = UDim2.new(0, 0, 1, 26)
    _KeyLbl.BackgroundTransparency = 1
    _KeyLbl.Text = "[L-CTRL]"
    _KeyLbl.TextColor3 = Color3.fromRGB(120, 120, 120)
    _KeyLbl.TextScaled = true
    _KeyLbl.Font = Enum.Font.Gotham
    _KeyLbl.Parent = _BtnFrame

    -- Draggable logic
    local _dragging = false
    local _dragStart, _startPos

    _BtnFrame.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1
        or inp.UserInputType == Enum.UserInputType.Touch then
            _dragging  = true
            _dragStart = inp.Position
            _startPos  = _BtnFrame.Position
        end
    end)
    _BtnFrame.InputEnded:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1
        or inp.UserInputType == Enum.UserInputType.Touch then
            _dragging = false
        end
    ...
    _UIS.InputChanged:Connect(function(inp)
        if _dragging and (inp.UserInputType == Enum.UserInputType.MouseMovement
        or inp.UserInputType == Enum.UserInputType.Touch) then
            local delta = inp.Position - _dragStart
            _BtnFrame.Position = UDim2.new(
    ...
    -- State
    local _windowVisible = true  -- Window đang hiện
    local _isAnimating   = false

    -- Hàm thực sự toggle Window Fluent
    local function _applyWindowVisible(show)
        pcall(function()
            -- Cách 1: dùng API Fluent nếu có
            if Window and Window.Enabled ~= nil then
                Window.Enabled = show
                return
            end
            -- Cách 2: toggle tất cả ScreenGui trừ nút toggle
            for _, gui in pairs(_PGui:GetChildren()) do
                if gui:IsA("ScreenGui") and gui.Name ~= "ToggleButtonGui" then
                    gui.Enabled = show
                end
            end
        end)
    ...
    -- Hàm flip animation + toggle window
    local function _flipToggle()
        if _isAnimating then return end
        _isAnimating = true

        -- Phase 1: thu nhỏ
        local shrink = _TweenSvc:Create(_BtnFrame,
        ...
        _applyWindowVisible(_windowVisible)
        ...
        if _windowVisible then
            ...
            _FrontFace.Visible = false
            _BackFace.Visible  = true
            _StatusLbl.Text       = "OFF"
            _StatusLbl.TextColor3 = Color3.fromRGB(255, 80, 80)
            _BackStroke.Color     = Color3.fromRGB(200, 50, 50)
        end

        -- Phase 2: mở rộng
        local expand = _TweenSvc:Create(_BtnFrame,
        ...
        -- Bounce nhẹ
        local bounce = _TweenSvc:Create(_BtnFrame,
        ...
        local restore = _TweenSvc:Create(_BtnFrame,
    ...
    -- Click vào nút cũng toggle
    _BtnFrame.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1
        or inp.UserInputType == Enum.UserInputType.Touch then
            -- Phân biệt click vs drag: chỉ toggle nếu không drag
    ...
    -- Glow pulse khi ON
    _RunSvc.Heartbeat:Connect(function()
        if _windowVisible and not _isAnimating then
            local alpha = (math.sin(tick() * 3) + 1) / 2
            _FrontStroke.Thickness = 2 + alpha * 1.5
        end
    end)
end
-- ============================================================
